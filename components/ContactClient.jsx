...
const MagneticBtn = ({ children, type = "button", disabled = false, style = {}, ...props }) => {
...
export default function ContactClient() {
  ...
  const handleSubmit = (e) => {
    ...
    const text = [
      `New Inquiry — ${form.service || 'General'}`,
      `Name: ${form.name}`,
      `Email: ${form.email}`,
    ...
    const whatsappUrl = `https://wa.me/971551171721?text=${encodeURIComponent(text)}`;
            ...
            <div style={{
              ...
              {submitted ? (
                <div style={{ textAlign: 'center', padding: 'clamp(2rem, 8vw, 3rem) 0' }}>
                  ...
                  <h3 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 'clamp(1.3rem, 4vw, 1.5rem)', marginBottom: '1rem', color: INK }}>WhatsApp opened!</h3>
                  <p style={{ color: INK60, lineHeight: 1.7, fontFamily: "'DM Sans', sans-serif" }}>
                    Your message is ready to send on WhatsApp. We'll be in touch within 24 hours to schedule your free growth call.
                  </p>
                </div>
              ) : (
                <form onSubmit={handleSubmit}>
                  ...
                  >
                    {loading ? 'Opening WhatsApp…' : <>Send via WhatsApp <ArrowUpRight size={16} /></>}
                  </MagneticBtn>
                </form>
              )}
            </div>
