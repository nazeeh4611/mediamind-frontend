...
const FALLBACK_WORKS = [
  ...
  // ... other fallback works with updated colors
];
...
function RelatedCard({ work }) {
  ...
  return (
    <Link to={`/works/${safeWork._id}`} style={{ display: 'block', textDecoration: 'none', background: WHITE, border: `1px solid ${INK10}`, borderRadius: '20px', overflow: 'hidden', transition: 'all 0.35s cubic-bezier(0.4,0,0.2,1)' }}
  ...
  if (error || !work) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: WHITE }}>
        ...
        <Link to="/works" style={{ color: ORANGE, textDecoration: 'none', fontWeight: 700, fontSize: '0.95rem' }}>← Back to Works</Link>
      </div>
    );
  }
        ...
        <section style={{ padding: '3rem 0 5rem', background: OFF_WHITE, borderTop: `1px solid ${INK10}` }}>
          <div className="container" style={{ maxWidth: 1200, margin: '0 auto', padding: '0 2rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
              ...
              <Link to="/works" style={{ color: ORANGE, textDecoration: 'none', fontSize: '0.85rem', fontWeight: 700 }}>View All →</Link>
            </div>
