        ...
        <div className="contact-right" style={{ opacity: 0 }}>
          <div style={{ background: WHITE, borderRadius: 24, padding: isMobile ? '1.75rem 1.25rem' : '2.5rem', boxShadow: '0 30px 80px rgba(0,0,0,0.3)' }}>
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
              {[{ label: 'Name*', key: 'name', placeholder: 'Jane Smith', type: 'text' }, { label: 'Email*', key: 'email', placeholder: 'jane@gmail.com', type: 'email' }].map(f => (
                <div key={f.key}>
                  ...
                  <input type={f.type} placeholder={f.placeholder} value={form[f.key]} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value }))} style={{ width: '100%', padding: '0.75rem 1rem', border: `1px solid ${INK10}`, borderRadius: 10, fontFamily: "'Inter', sans-serif", fontSize: '0.88rem', color: INK, background: OFF_WHITE, outline: 'none', boxSizing: 'border-box' }} />
                </div>
              ))}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
              <div>
                ...
                <div style={{ display: 'flex', border: `1px solid ${INK10}`, borderRadius: 10, overflow: 'hidden', background: OFF_WHITE }}>
                  ...
                  <input type="tel" placeholder="+971" value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} style={{ flex: 1, padding: '0.75rem', border: 'none', background: 'transparent', fontFamily: "'Inter', sans-serif", fontSize: '0.88rem', color: INK, outline: 'none' }} />
                </div>
              </div>
              <div>
                ...
                <input type="url" placeholder="www.example.com" value={form.website} onChange={e => setForm(p => ({ ...p, website: e.target.value }))} style={{ width: '100%', padding: '0.75rem 1rem', border: `1px solid ${INK10}`, borderRadius: 10, fontFamily: "'Inter', sans-serif", fontSize: '0.88rem', color: INK, background: OFF_WHITE, outline: 'none', boxSizing: 'border-box' }} />
              </div>
            </div>
            <div style={{ marginBottom: '1.25rem' }}>
              ...
              <textarea rows={4} placeholder="Enter your message" value={form.message} onChange={e => setForm(p => ({ ...p, message: e.target.value }))} style={{ width: '100%', padding: '0.75rem 1rem', border: `1px solid ${INK10}`, borderRadius: 10, fontFamily: "'Inter', sans-serif", fontSize: '0.88rem', color: INK, background: OFF_WHITE, outline: 'none', resize: 'none', boxSizing: 'border-box' }} />
            </div>
            <button style={{ width: '100%', padding: '1rem', background: INK, color: WHITE, border: 'none', borderRadius: 12, fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: '1rem', cursor: 'pointer', transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = '#333'} onMouseLeave={e => e.currentTarget.style.background = INK}>
              Submit
            </button>
          </div>
        </div>
