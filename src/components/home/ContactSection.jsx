...
export function ContactSection() {
  ...
  useEffect(() => {
    ...
    gsap.fromTo([left, right],
      { opacity: 0, y: 50 },
      { opacity: 1, y: 0, duration: 0.8, stagger: 0.15, ease: 'power3.out', scrollTrigger: { trigger: sectionRef.current, start: 'top 75%', once: true } }
    );
  }, []);
  ...
  return (
    <section ref={sectionRef} style={{ position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 2, maxWidth: 1200, margin: '0 auto', padding: '8rem 2rem 5rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'center' }}>
        <div className="contact-left" style={{ opacity: 0 }}>
          <div style={{ width: 60, height: 60, background: 'rgba(255,255,255,0.1)', borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '2rem', fontSize: '1.8rem' }}>📁</div>
          <h2 style={{ fontFamily: "'Inter', sans-serif", fontWeight: 800, fontSize: 'clamp(2rem,4vw,3.5rem)', color: WHITE, lineHeight: 1.1, letterSpacing: '-0.03em', marginBottom: '1.25rem' }}>
            Have a project<br />in mind? 👋
          </h2>
          <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '1rem', color: 'rgba(255,255,255,0.65)', lineHeight: 1.75, marginBottom: '1rem' }}>
            Connect with our team of dazzling designers and creative developers.
          </p>
          <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '1rem', color: 'rgba(255,255,255,0.5)' }}>
            Catch us for coffee, it's always on us ☕
          </p>
        </div>
        <div className="contact-right" style={{ opacity: 0 }}>
          <div style={{ background: WHITE, borderRadius: 24, padding: '2.5rem', boxShadow: '0 30px 80px rgba(0,0,0,0.3)' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
              {[
                { label: 'Name*', key: 'name', placeholder: 'Jane Smith', type: 'text' },
                { label: 'Email*', key: 'email', placeholder: 'jane@gmail.com', type: 'email' },
              ].map(f => (
                <div key={f.key}>
                  ...
                  <input
                    ...
                    style={{
                      width: '100%', padding: '0.75rem 1rem', border: `1px solid ${INK10}`,
                      borderRadius: 10, fontFamily: "'Inter', sans-serif", fontSize: '0.88rem', color: INK,
                      background: OFF_WHITE, outline: 'none', boxSizing: 'border-box',
                    }}
                  />
                </div>
              ))}
            ...
            <div style={{ marginBottom: '1.25rem' }}>
              ...
              <textarea
                ...
                style={{
                  width: '100%', padding: '0.75rem 1rem', border: `1px solid ${INK10}`,
                  borderRadius: 10, fontFamily: "'Inter', sans-serif", fontSize: '0.88rem', color: INK,
                  background: OFF_WHITE, outline: 'none', resize: 'none', boxSizing: 'border-box',
                }}
              />
            </div>
            <button
