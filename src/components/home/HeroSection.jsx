...
import { GRAD_HERO, INK, INK60, INK30, INK10, ORANGE, WHITE, heroWords } from '../../utils/constants';
...
export function HeroSection({ onEnterBtn, onLeaveBtn }) {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    gsap.fromTo(el, { opacity: 0, y: 60 }, { opacity: 1, y: 0, duration: 1, ease: 'power3.out', delay: 0.2 });
  ...
  const dynamicFontSize = charCount > 16
    ? 'clamp(1.8rem,4.5vw,4.2rem)'
    : charCount > 12
    ? 'clamp(2.2rem,5.5vw,5rem)'
  ...
  return (
    <section style={{
      ...
      <div style={{ position: 'absolute', width: 400, height: 400, borderRadius: '50%', background: 'radial-gradient(circle,rgba(139,92,246,.08) 0%,transparent 70%)', top: '30%', right: '20%', pointerEvents: 'none' }} />

      <div style={{ textAlign: 'center', position: 'relative', zIndex: 2, maxWidth: 1000, padding: '0 2rem', width: '100%' }}>
        <div ref={titleRef} style={{ opacity: 0 }}>
          ...
          <div style={{
            ...
            <h1 style={{
              ...
              transition: 'transform 0.4s cubic-bezier(0.4,0,0.2,1), opacity 0.35s ease, font-size 0.2s ease',
              whiteSpace: 'nowrap',
              display: 'block',
            }}>
              {currentWord}
            ...
            <div style={{ display: 'flex' }}>
              {[0, 1, 2].map((i) => (
                <div key={i} style={{
                  ...
                  background: `hsl(${i * 60 + 200},70%,65%)`,
                  border: '2px solid white',
                  marginLeft: i > 0 ? -10 : 0,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontFamily: "'Inter', sans-serif", fontWeight: 700, color: WHITE, fontSize: '0.75rem',
                }}>
                  {['A', 'B', 'C'][i]}
                </div>
              ))}
              <div style={{
                ...
                <motion.div
                  ...
                  style={{ width: 16, height: 16, borderRadius: '50%', border: '2px solid #374151', borderTopColor: 'transparent' }}
                />
              </div>
            </div>
            <span style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.88rem', color: INK60, fontWeight: 500 }}>200+ already with us</span>
          </div>
        </div>
        ...
        <div ref={statsRef} style={{
          ...
          paddingTop: '2rem', borderTop: `1px solid ${INK10}`,
        }}>
          {[{ v: '200+', l: 'Clients' }, { v: '98%', l: 'Retention' }, { v: '8×', l: 'ROAS' }, { v: '$50M+', l: 'Revenue' }].map((s, i) => (
            <div key={i} style={{ textAlign: 'center' }}>
              <div style={{ fontFamily: "'Inter', sans-serif", fontWeight: 800, fontSize: 'clamp(1.6rem,3vw,2.2rem)', color: INK }}>{s.v}</div>
              <div style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.7rem', color: INK30, textTransform: 'uppercase', letterSpacing: '0.1em', marginTop: '0.2rem' }}>{s.l}</div>
            </div>
          ))}
        </div>
      </div>
